...
const searchEvents = async () => {
  try {
    const response = await eventStore.getEventsByName(searchQuery.value);
      ...
      <div class="collapse navbar-collapse" id="navbarSupportedContent">
        ...
        <div class="input-group justify-content-center">
          <div class="form-outline bg-white rounded" data-mdb-input-init>
            <input v-model="searchQuery" type="search" id="form1" class="form-control" @input="searchEvents" />
            <label class="form-label" for="form1">Search</label>
          </div>
          <button type="button" class="btn btn-primary" data-mdb-ripple-init @click="searchEvents">
        ...
        <ul class="navbar-nav mb-2 mb-lg-0" v-if="authStore.loggedInUser">
          <li class="nav-item dropdown custom-margin-right">
            ...
            <ul class="dropdown-menu dropdown-menu-end">
              ...
              <li>
                <button class="btn btn-outline-danger btn-sm m-lg-2" @click="onLogOut()">
                  Log Out
                </button>
              </li>
            </ul>
