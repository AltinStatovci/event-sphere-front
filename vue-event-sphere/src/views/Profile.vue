  ...
  const paginatedUsers = computed(() => {
    const startIndex = (currentPageUsers.value - 1) * pageSizeUsers;
    return sortedUsers.value.slice(startIndex, startIndex + pageSizeUsers);
  });

  const totalPagesUsers = computed(() => Math.ceil(users.value.length / pageSizeUsers));

  const setCurrentPageUsers = (page) => {
    currentPageUsers.value = page;
  };

  watch(() => users.value, () => {
    setCurrentPageUsers(1); // Reset to first page when users change
  });
            ...
            <div class="card-body">
              ...
              <nav v-if="users.length > 0">
                <ul class="pagination justify-content-center">
                  <li
                      class="page-item"
                      v-for="page in totalPagesUsers"
                      :key="page"
                      :class="{ active: currentPageUsers === page }"
                  >
                    <a class="page-link" href="#" @click.prevent="setCurrentPageUsers(page)">{{ page }}</a>
                  </li>
                </ul>
              </nav>
            </div>
