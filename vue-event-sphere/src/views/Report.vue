...
const fetchReports = async () => {
  try {
    const response = await axios.get(`http://localhost:5220/api/Report`);
    if (!response.ok) {
...
const submitComplaint = async () => {
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to ${method === "POST" ? "submit" : "update"} report: ${
          response.statusText
        }`
      );
    }
    ...
    console.log(
      `${
        method === "POST" ? "Report submitted" : "Report updated"
      } successfully`
    );
    ...
    console.error(
      `Error ${method === "POST" ? "submitting" : "updating"} report:`,
      error.message
    );
  }
};
...
<template>
  <div class="d-flex">
    ...
    <div
      class="dashboard"
      :style="{ backgroundColor: isDarkMode ? '#0f172a' : '#3fa387' }"
    >
      <!-- Write Report Button -->
      <button class="write-report-btn" @click="openModal">
        <span class="material-symbols-outlined icon" style="font-size: 2rem"
          >edit_note</span
        >
        <p>Write Report</p>
      </button>
      ...
      <h1>
        Hello,
        {{
          authstore.isLoggedIn && authstore.email
            ? `User: ${authstore.id}`
            : "Guest"
        }}
      </h1>
        ...
        <div class="modal-content">
          ...
          <div class="complaint-form">
            <span
              ...
              style="font-size: 3rem; margin-left: 260px"
              >report</span
            >
            <h1>{{ modalTitle }}</h1>
            ...
            <!-- Submit Button -->
            <button class="submit-btn" @click="submitComplaint">
              {{ modalButton }}
            </button>
          </div>
        </div>
      ...
      <div class="report-cards-container">
        ...
        >
          <div class="card-header">
            ...
            <p class="card-email">{{ report.userEmail }}</p>
          </div>
          <div class="card-body">
            <p>{{ report.reportDesc }}</p>
            <p>Status: {{ report.reportAnsw }}</p>
            <span
              v-if="report.reportAnsw === 'Waiting for Respond'"
              class="material-symbols-outlined icon"
              >error</span
            >
          ...
          <button
            class="respond-btn"
            :style="{
              border: '7px solid ' + (isDarkMode ? '#0f172a' : '#3fa387'),
            }"
